...
# Residue
EoM_objective = lambda l_over_lb, vr_over_cs, Δf0_over_b, ΔT : K_Δτ(l_over_lb, vr_over_cs, Δf0_over_b) + K_ΔT(l_over_lb, ΔT) - Kc(vr_over_cs)
...
def initialize(l_over_lb, ΔT, method='brentq'):
    ...
    if (EoM_objective(l_over_lb, vmin_over_cs, Δf0_over_b, ΔT) * EoM_objective(l_over_lb, vmax_over_cs, Δf0_over_b, ΔT) <= 0):
        # Function
        func = lambda vr_over_cs : EoM_objective(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
        # Gradient
        grad = lambda vr_over_cs : EoM_dv(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
...
# Find rupture velocity for v_r/c_s for any crack length l/l_b - used only to initialize the ODE
def solve_root_in_l(l_over_lb, v_guess_over_cs, Δf0_over_b, ΔT):
    # Function
    func = lambda vr_over_cs : EoM_objective(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
    # Gradient
    grad = lambda vr_over_cs : EoM_dv(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
...
def solve_root_in_v(vr_over_cs, l_guess_over_lb, Δf0_over_b, ΔT):
    # Function
    func = lambda l_over_lb : EoM_objective(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
    # Gradient
    grad = lambda l_over_lb : EoM_dl(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
    # Solve for root of the EoM
...
def solve_ode_in_l(l_ini_over_lb, l_fin_over_lb, ΔT, N_steps=1000, Δl_over_lb = 1E-6, rtol=1E-9, atol=1E-9, max_steps=10000):
    ...
    def formulate_ode_in_l(l_over_lb, y, args):
        ...
        Fl = EoM_dl(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
        Fv = EoM_dv(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
        Fd = EoM_df(l_over_lb, vr_over_cs, Δf0_over_b, ΔT)
        ...
        if (sol.result == dfx.RESULTS.max_steps_reached):
            ...
            for i in range(1, N_cont):
                ...
                while Err_l > 1e-8:
                    ...
                    l_guess_old = solve_root_in_v(v_cont_over_cs[i], l_cont_over_lb[i-1], Δf0_over_b_guess, ΔT)
