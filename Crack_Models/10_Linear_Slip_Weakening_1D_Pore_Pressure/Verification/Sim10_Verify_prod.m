...
for i = 1:length(files)
    if Results(i).ssy == 0
        ...
        if Results(i).taub == 0.744
            [t_dyn,ind] = max(Results(i).time);
            plot(t_dyn,Results(i).a(ind),'o','LineWidth',Plotting.lwidth_1col,'Color','k','MarkerFaceColor','w')
        end
    end
end
...
txt = text(3.5,27,'$$\tilde{\tau}_{\mathrm{b}}=0.744$$');
set(txt,'Interpreter','latex','fontsize',Plotting.fsize_1col)

txt = text(10,27,'$$\tilde{\tau}_{\mathrm{b}}=0.696$$');
set(txt,'Interpreter','latex','fontsize',Plotting.fsize_1col)
