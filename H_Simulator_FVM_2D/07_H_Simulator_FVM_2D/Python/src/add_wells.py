...
def add_wells(Flow,State,Wells):
    ...
    #Find permeability
    _,_,keff,_,_,dkeff_dP = perm(Flow,State)
    ...
    Q = keff * (Pwf - State["P"]) * WI * Rho / Flow["muf"] #Flow rate [kg/sec]
    dQdP = keff * WI * (dRhodP * (Pwf - State["P"]) - Rho) / Flow["muf"] + dkeff_dP * WI * Rho / Flow["muf"]  #Derivative wrt pressure [kg/(Pa sec)]
